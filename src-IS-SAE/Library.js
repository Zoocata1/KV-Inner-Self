...
// ============================================================================
// Story Arc Engine (SAE) — KV-safe integration addon
// Based on Story Arc Engine by Yi1i1i.
// State is namespaced under state.SAE and all context changes are append-only.
// ============================================================================

state.SAE = (state.SAE && typeof state.SAE === "object" && !Array.isArray(state.SAE))
  ? state.SAE
  : {};
state.SAE.version = "IS-SAE-KV.1";

const SAE_DEFAULT_ARC_PROMPT = `
<<</SYSTEM>  
- Stop the story.  
...
function SAE_default(key, value) {
  if (state.SAE[key] === undefined) state.SAE[key] = value;
  return state.SAE[key];
}

SAE_default("stop", false);
SAE_default("unlockFeedAIPrompt", false);
SAE_default("saveOutput", false);
SAE_default("storyArc", "");
SAE_default("attemptCounter", 0);
SAE_default("turnNum", 1);
SAE_default("attemptLimit", 3);
SAE_default("turnsPerElemRemoval", 3);
SAE_default("turnsPerAICall", 35);
SAE_default("arcPrompt", SAE_DEFAULT_ARC_PROMPT);
SAE_default("contextMode", "normal");

function SAE_findCardIndex(keys) {
  return storyCards.findIndex(card => (
    card && typeof card === "object" && card.keys === keys
  ));
}

function SAE_ensureCard(keys, entry) {
  let index = SAE_findCardIndex(keys);
  if (index !== -1) return index;
  try {
    const created = addStoryCard(keys, entry, "class");
    if (created === false) return SAE_findCardIndex(keys);
    return Number.isInteger(created) ? created : SAE_findCardIndex(keys);
  } catch (e) {
    log("SAE addStoryCard error:", e);
    return -1;
  }
}

function SAE_updateCard(index, keys, entry) {
  if (!Number.isInteger(index) || index < 0 || index >= storyCards.length) return false;
  try {
    updateStoryCard(index, keys, entry, storyCards[index]?.type || "class");
    return true;
  } catch (e) {
    log("SAE updateStoryCard error:", e);
    return false;
  }
}

function SAE_settingsText() {
  return [
    `stop_SAE = ${state.SAE.stop}`,
    `turnsPerAICall = ${state.SAE.turnsPerAICall}`,
    `attemptLimit = ${state.SAE.attemptLimit}`,
    `turnsPerElemRemoval = ${state.SAE.turnsPerElemRemoval}`,
    `arcPrompt = ${state.SAE.arcPrompt}`
  ].join("\n");
}

function SAE_retrieveSettings() {
  const index = SAE_findCardIndex("Story Arc Settings");
  if (index === -1) return;
  const entry = typeof storyCards[index]?.entry === "string" ? storyCards[index].entry : "";

  const stopMatch = entry.match(/stop_SAE\s*=\s*([A-Za-z]+)/);
  if (stopMatch) state.SAE.stop = stopMatch[1].toLowerCase() === "true";

  const turnsMatch = entry.match(/turnsPerAICall\s*=\s*(\d+)/);
  if (turnsMatch) state.SAE.turnsPerAICall = Math.max(1, Number(turnsMatch[1]) || 35);

  const attemptMatch = entry.match(/attemptLimit\s*=\s*(\d+)/);
  if (attemptMatch) state.SAE.attemptLimit = Math.max(1, Number(attemptMatch[1]) || 3);

  const removalMatch = entry.match(/turnsPerElemRemoval\s*=\s*(\d+)/);
  if (removalMatch) state.SAE.turnsPerElemRemoval = Math.max(0, Number(removalMatch[1]) || 0);

  const promptMatch = entry.match(/arcPrompt\s*=\s*(<<[\s\S]*?>>)/);
  if (promptMatch) state.SAE.arcPrompt = promptMatch[1];
}

function SAE_initLibrary() {
  try {
    const settingsIndex = SAE_ensureCard("Story Arc Settings", SAE_settingsText());
    SAE_retrieveSettings();
    if (settingsIndex !== -1) {
      SAE_updateCard(settingsIndex, "Story Arc Settings", SAE_settingsText());
    }

    const arcIndex = SAE_ensureCard("/Current Story Arc", state.SAE.storyArc);
    if (arcIndex !== -1) {
      const entry = storyCards[arcIndex]?.entry;
      if (typeof entry === "string") state.SAE.storyArc = entry;
      SAE_updateCard(arcIndex, "/Current Story Arc", state.SAE.storyArc);
    }
  } catch (e) {
    log("SAE library init error:", e);
  }
}

function SAE_storeArc() {
  const index = SAE_ensureCard("/Current Story Arc", state.SAE.storyArc);
  if (index !== -1) SAE_updateCard(index, "/Current Story Arc", state.SAE.storyArc);
}

function SAE_appendWithinBudget(text, suffix, requireWhole = true) {
  if (typeof suffix !== "string" || suffix === "") return text;
  const room = Number.isInteger(info.maxChars)
    ? Math.max(0, info.maxChars - text.length - 16)
    : suffix.length;
  if (suffix.length <= room) return text + suffix;
  return requireWhole ? text : text + suffix.slice(0, room);
}

function onInput_SAE(text) {
  if (state.SAE.stop === true) return text;

  if (text.includes("/help sae")) {
    state.SAE.commandCenter = [
      "STORY ARC ENGINE By Yi1i1i",
      "Story Arc Engine periodically calls the AI to create a future story arc.",
      "Edit the Story Arc Settings card to configure it.",
      "Commands: /redo arc, /stop, /help sae"
    ].join("\n");
    return " ";
  }

  if (text.includes("/redo arc")) {
    state.SAE.unlockFeedAIPrompt = true;
    state.SAE.saveOutput = true;
    state.SAE.attemptCounter = 0;
    return "<< ➰ Regenerating Story Arc... >>";
  }

  if (text.includes("/stop") && state.SAE.saveOutput === true) {
    state.SAE.unlockFeedAIPrompt = false;
    state.SAE.saveOutput = false;
    state.SAE.attemptCounter = 0;
    return "<< ⛔ Story Arc Generation Stopped. >>";
  }
...
function onContext_SAE(text) {
  state.SAE.contextMode = "normal";
  if (state.SAE.stop === true) return text;

  if (state.SAE.commandCenter) {
    state.SAE.contextMode = "command";
    return text;
  }

  if (state.SAE.unlockFeedAIPrompt === true) {
    const suffix = "\n\n" + String(state.SAE.arcPrompt || SAE_DEFAULT_ARC_PROMPT).trim() + "\n\n";
    const appended = SAE_appendWithinBudget(text, suffix, true);
    if (appended === text) {
      state.SAE.contextMode = "deferred";
      log("IS+SAE KV: arc-generation prompt deferred; no append budget.");
      return text;
    }
    state.SAE.unlockFeedAIPrompt = false;
    state.SAE.contextMode = "arc";
    return appended;
  }

  if (state.SAE.saveOutput === false && typeof state.SAE.storyArc === "string" && state.SAE.storyArc.trim() !== "") {
    const suffix = "\n\n<SYSTEM>\n# STORY ARC GUIDANCE\n"
      + state.SAE.storyArc.trim()
      + "\n</SYSTEM>\n\n";
    const appended = SAE_appendWithinBudget(text, suffix, true);
    if (appended === text) log("IS+SAE KV: story arc skipped; no append budget.");
    return appended;
  }
...
function SAE_saveStoryArc(text) {
  if (state.SAE.saveOutput !== true) return text;

  const outputtedArc = SAE_cleanArc(text);
  const lineCount = outputtedArc === "" ? 0 : outputtedArc.split("\n").length;

  if (lineCount < 8) {
    if (state.SAE.attemptCounter >= state.SAE.attemptLimit) {
      state.SAE.saveOutput = false;
      state.SAE.unlockFeedAIPrompt = false;
      state.SAE.attemptCounter = 0;
    ...
    state.SAE.unlockFeedAIPrompt = true;
    state.SAE.saveOutput = true;
    state.SAE.attemptCounter += 1;
    return `\n<< ⏳ Generating Story Arc (Attempt ${state.SAE.attemptCounter}/${state.SAE.attemptLimit})... Click 'Continue' or type '/stop'. >>`;
  }

  state.SAE.attemptCounter = 0;
  state.SAE.storyArc = "Write the story in the following direction:\n" + outputtedArc;
  state.SAE.saveOutput = false;
  state.SAE.unlockFeedAIPrompt = false;
  SAE_storeArc();
...
function SAE_callAIForArc(text) {
  if (
    state.SAE.saveOutput === false
    && (state.SAE.turnNum === 1 || state.SAE.turnNum % state.SAE.turnsPerAICall === 0)
  ) {
    state.SAE.unlockFeedAIPrompt = true;
    state.SAE.saveOutput = true;
...
function SAE_removeFirstListItem(text) {
  const lines = String(text || "").split("\n");
  const start = lines.findIndex(line => /^\d+\.\s/.test(line));
  if (start === -1) return text;
  const before = lines.slice(0, start);
  const rest = lines.slice(start + 1).map((line, i) => (
    `${i + 1}. ${line.replace(/^\d+\.\s*/, "")}`
  ));
  return [...before, ...rest].join("\n");
}

function SAE_arcElementRemoval() {
  if (
    state.SAE.saveOutput === false
    && state.SAE.turnNum >= 5
    && state.SAE.turnsPerElemRemoval !== 0
    && state.SAE.turnNum % state.SAE.turnsPerElemRemoval === 0
  ) {
    state.SAE.storyArc = SAE_removeFirstListItem(state.SAE.storyArc);
    SAE_storeArc();
  }
}

function onOutput_SAE(text) {
  if (state.SAE.stop === true) {
    state.SAE.contextMode = "normal";
    return text;
  }

  if (state.SAE.commandCenter) {
    const out = state.SAE.commandCenter;
    delete state.SAE.commandCenter;
    state.SAE.contextMode = "normal";
    return out || " ";
  }

  if (state.SAE.contextMode === "deferred") {
    // This output was ordinary story text because the arc task could not fit.
    state.SAE.contextMode = "normal";
    state.SAE.turnNum += 1;
    return text;
  }

  if (state.SAE.contextMode === "arc" || state.SAE.saveOutput === true) {
    text = SAE_saveStoryArc(text);
  } else {
    text = SAE_callAIForArc(text);
    SAE_arcElementRemoval();
  }
